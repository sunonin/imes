...
if (isset($_GET['filterSchoolYear']) || isset($_GET['filterCompany']) || isset($_GET['filterProgram']) || isset($_GET['filterSection']) || isset($_GET['filterName'])) {
	if (!empty($filterPrograms)) {
		$section_opts = json_decode($imes->fetchProgramOpts2($filterPrograms), true);
	}
	$students = json_decode($imes->fetchStudents2($filterPrograms, $filterSections, $filterNames, $filterSchoolYear, $filterCompany), true);
}
